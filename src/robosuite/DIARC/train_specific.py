...
def train_reach_drop(env, eval_env):
    ...
    model = SAC.load("./models/ReachDrop/1/full/reachdrop_sac.zip", env=env)
...
def train_drop(env, eval_env):
    ...
    ####NEED TO UPDATE FOR DROP
    model = SAC.load("./models/ReachDrop/1/full/reachdrop_sac.zip", env=env)
    model.load_replay_buffer("./models/ReachDrop/1/full/drop_sac_replay_buffer.pkl")
    #Load replay buffer
    print("Loaded model and replay buffer")
    print(f"Training for {TRAINING_STEPS}")
